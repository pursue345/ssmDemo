...
//        final int CONST_XIE = 14;// 斜方向移动的路径评分移动的路径评分
        ...
        //根据起点到终点的坐标，选择算法的方向  1-左下  2-下  3-右下  4-左 6-右  7-左上  8-上 9-右上
//        String directStr = findDirect(startPoint.x,startPoint.y, endPoint.x,endPoint.y);
        ...
        while (!openList.isEmpty() && !openList.contains(endPoint)) {
            ...
            //根据起点到终点的坐标，选择算法的方向  1-左下  2-下  3-右下  4-左 6-右  7-左上  8-上 9-右上
//            String directStr = findDirect(curNode.x, curNode.y, endPoint.x, endPoint.y);

            //1-左下  2-下  3-右下  4-左 6-右  7-左上  8-上 9-右上
//            for (int i = 0; i < directStr.length(); i++) {
//                switch (directStr.charAt(i)) {
//                    case '1':
//                        // 左下
...
// }
    ...
    //根据起始点、终点坐标 x、y的差值，动态定义寻找方向的顺序 //1-左下  2-下  3-右下  4-左 6-右  7-左上  8-上 9-右上
    public static String findDirect(int x1, int y1, int x2, int y2) {
        int n;
        if (y1 == y2) {
            n = 0;
        } else {
...
// 不存在，添加到开启列表中
    ...
    private static void countH(int x, int y, Node node, Node eNode) {
        node.setF((int) Math.sqrt( Math.pow(node.getX() - eNode.getX(),2) + Math.pow(node.getY()- eNode.getY(),2)));
...
// }
    ...
    private class Node {
        ...
        private int f;// f=g+h
        private boolean isFirstDown;//是否是计算过进入雷区   true-计算过 false-没计算过
